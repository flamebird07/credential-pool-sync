...
def main():
    ...
    valid_records = [
        ...
        if record.get("model") and record.get("base_url") and record.get("api_key")
    ]
    if not valid_records:
        print("ERROR: 凭证池中没有有效记录", file=sys.stderr)
        sys.exit(1)

    try:
        current_config = get_current_model_config()
    except (OSError, yaml.YAMLError) as exc:
        print("ERROR: 无法读取当前主模型配置: {}".format(exc), file=sys.stderr)
        sys.exit(1)

    current_index = next(
        (
            index for index, record in enumerate(valid_records)
            if record["api_key"] == current_config["api_key"]
        ),
        None,
    )
    if current_index is None:
        ...
        if len(valid_records) == 1:
            print("无可用切换：凭证池中只有当前凭证")
            return
        target_record = valid_records[(current_index + 1) % len(valid_records)]
    ...
    print("切换成功: {}".format(target_record.get("label") or target_record["model"]))
