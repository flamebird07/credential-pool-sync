...
def auth_list():
    ...
    for line in r.stdout.splitlines():
        ...
        if m2 and provider:
            ...
            all_creds.append({"provider": provider, "idx": idx, "label": label, "active": is_active})
            if is_active: active = {"provider": provider, "idx": idx, "label": label}
    return provider, active, all_creds
