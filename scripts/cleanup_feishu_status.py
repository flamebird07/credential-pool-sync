#!/usr/bin/env python3
"""Reconcile Feishu credential status with the active Hermes model v7.3.0 — 飞书状态管理优化 + Provider 反推修复"""
...
def identity(api_key, base_url, model):
    return (api_key, str(base_url or "").strip().lower().rstrip("/"), str(model or "").strip().lower())
...
def cleanup_feishu_status():
    ...
    for record in records:
        ...
        if is_valid:
            ...
            new_note = current_note or error or "验证失败"
