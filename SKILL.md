...
Priority is normalized to tiers 0-9 (0 highest, 9 lowest). Synchronization and Claude Code proxy both load only the highest tier containing a usable credential, rotate in Feishu return order within that tier, and advance to the next tier only when the active tier is exhausted. `switch_next.py` identifies the current credential by the `(model, api_key, base_url)` triple-key (model + API key + base URL, all normalized). This is the user's preferred uniqueness rule — see "Pitfall: config.yaml 'default' key vs 'model' key identity mismatch" below for why this matters. Candidates are ordered by Feishu return order (not priority), and each candidate is health-checked before switching. `config.yaml` is written atomically with a UUID temporary file, flush, `fsync`, `os.replace`, and a Windows `msvcrt.locking` lock.
...
│   ├── v7.3.3-status-field-agent-disp...
...
└── scripts/
    ├── auto_bootstrap.py
    ├── claude_credential_proxy.py
    ├── cleanup_feishu_status.py
    ├── disable_claude_credential_pool.py
    ├── enable_claude_credential_pool.py
    ├── setup.py
    ├── switch_claude_credential.py
    ├── switch_next.py
...
- **Fixed Claude Code credential pool Feishu status display and priority-aware switching (P1-P31):**
  - **Status display (P1-P3, P8, P10, P12-P15):** Fixed active credential being excluded from healthy pool (`🔄 Claude Code 使用中` not recognized as healthy → `healthy`/`by_tier`/`_credentials` missing active, `_index` reset); fixed `_health_filter` overwriting in-use with `✅ 正常`; added `next_after`/`rotate` clearing of previous `Claude Code` marker; added pending-identity tracking for failed Feishu writes; fixed `_write_ui_state` dedup and multi-Agent merge (`_status_add`/`_status_remove` with strip, `_read_record_status`); fixed `_probe_credential` 401/403 retry and failure counters.
  - **Priority tier switching (P4-P7, P11, P19-P22, P26-P28):** Fixed `refresh()` overwriting `_advance_tier` result (now preserves current healthy tier, only advances when exhausted); fixed `_select_active_tier` receiving empty `_bad` (now derives `selectable_by_tier` from `healthy`); unified `priority()` to accept numeric `1.0`, rich-text `[{"text": "3"}]` and clamp 0-9; added `group_by_tier` ordering by `(priority, stable_order)`; added `switch_next.first_healthy` reuse of `health_results`; fixed `enable` table-id stable lookup and `_advance_tier` / `rotate` 503 control-plane auth.
  - **Cross-script consistency (P16-P18, P23-P25):** Fixed `switch_next`/`auto_bootstrap` identity normalization (`pool_identity`), round-robin within tier, `cleanup_feishu_status` two-phase full-tier health check + `reconcile_agent_status`, URL healing after compare.
  - **Docs:** Removed duplicate SKILL.md body (v7.21.0 copy), synced frontmatter `7.23.0` ↔ title `v7.23.0`, fixed `Rotation behavior` and `Files` tree to list Claude scripts.
  - **Verified:** `py_compile` 8/8 scripts, 11-step smoke test, 3 regression fixes (R1 `selectable_by_tier` source, R2 post-merge dedup, R3 failure counters, R4 `_agents` strip).
- **Synced to Hermes:** Workdir `57172`/`57695` ↔ Hermes skill, single source of truth restored.
